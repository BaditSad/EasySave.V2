...
namespace CryptoSoft
{
    class Menu
    {
        ...
        public void print3(string lang)
        {
            if (lang == "en")
            {
                Console.WriteLine("\nNo file matched the selected extensions configured in EasySave3.0!");
                for (int i = 5; i > 0; i--)
                {
                    Console.WriteLine("\rApplication terminate in {0}s!", i);
                }
            }

            if (lang == "fr")
            {
                Console.WriteLine("\nAucun fichier trouvé qui correspond aux extensions configuré dans EasySave3.0 !");
                for (int i = 5; i > 0; i--)
                {
                    Console.WriteLine("\rL'application va se fermer dans {0}s !", i);
                }
            }
        }
